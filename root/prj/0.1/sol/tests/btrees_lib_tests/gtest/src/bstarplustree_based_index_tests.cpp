 ...
 */
class BStarPlusTreeBasedIndexTest : public ::testing::Test
{
...
public:
     ...
     */
    BStarPlusTreeBasedIndexTest() { setlocale(LC_ALL, "Russian"); }
...
}; // class BStarPlusTreeBasedIndexTest

void BStarPlusTreeBasedIndexTest::testIndex(std::string fileName, std::string treeFileName,
        std::string indexOutFileName, std::string linearOutFileName,
        std::wstring name, int expectedCount)
{
    ...
    indexer.create(BaseBTree::TreeType::B_STAR_PLUS_TREE, ORDER, treeFileName);
...
void BStarPlusTreeBasedIndexTest::testIndexWithDifferentParams(std::string fileName, std::string treeFileName,
        std::string indexOutFileName, std::string outputCsvFileName, std::wstring name, int expectedCount)
{
    std::cout << "File " << fileName << std::endl;
    ...
    for (int i = 0; i < ORDERS_LENGTH; ++i)
    {
        ...
        btree::Indexer indexer;
        indexer.create(BaseBTree::TreeType::B_STAR_PLUS_TREE, ORDERS[i], treeFileName);
...
void BStarPlusTreeBasedIndexTest::testIndexWithDifferentParamsAndCountAggregates(std::string fileName, std::string treeFileName,
        std::string indexOutFileName, std::string outputCsvFileName, std::wstring name, int expectedCount)
{
    std::cout << "File " << fileName << std::endl;
...
std::list<std::wstring> BStarPlusTreeBasedIndexTest::searchLinearly(std::wstring& name, std::string fileName)
{
...
void BStarPlusTreeBasedIndexTest::writeOccurrencesToFile(std::list<std::wstring>& occurrences, std::string& fileName)
{
...
double BStarPlusTreeBasedIndexTest::getTimeInSecs(clock_t begin, clock_t end)
{
...
TEST_F(BStarPlusTreeBasedIndexTest, IndexerTest1)
{
...
TEST_F(BStarPlusTreeBasedIndexTest, IndexerTest2)
{
...
TEST_F(BStarPlusTreeBasedIndexTest, IndexerWithDifferentParamsTest1)
{
    testIndexWithDifferentParams(
            "University_and_books.csv", "BTree_University_and_books.xibt",
            "Occurrences_index_University_and_books.txt", "Results_BStarPlusTree_University_and_books.csv",
            L"Подбельский", 3
    );
}

TEST_F(BStarPlusTreeBasedIndexTest, IndexerWithDifferentParamsTest2)
{
    testIndexWithDifferentParams(
            "Hospital_log.csv", "BTree_Hospital_log.xibt",
            "Occurrences_index_Hospital_log.txt", "Results_BStarPlusTree_Hospital_log.csv",
            L"1e consult poliklinisch", 1136
    );
}

TEST_F(BStarPlusTreeBasedIndexTest, IndexerWithDifferentParamsAndAggregatesTest1)
{
    testIndexWithDifferentParamsAndCountAggregates(
            "University_and_books.csv", "BTree_University_and_books.xibt",
            "Occurrences_index_University_and_books.txt", "Results_Aggr_BStarPlusTree_U_a_b.csv", // Because of too long file name.
            L"Подбельский", 3
    );
}

TEST_F(BStarPlusTreeBasedIndexTest, DISABLED_IndexerWithDifferentParamsAndAggregatesTest2) // TODO: Enable.
{
    testIndexWithDifferentParamsAndCountAggregates(
            "Hospital_log.csv", "BTree_Hospital_log.xibt",
            "Occurrences_index_Hospital_log.txt", "Results_Aggr_BStarPlusTree_Hospital_log.csv",
            L"1e consult poliklinisch", 1136
    );
}
